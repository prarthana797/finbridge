...
export default function Navbar() {
  ...
  function goToDashboard() {
    ...
    } else if (isAgent || currentUser?.role === 'agent') {
    ...
    <header className="navbar-white py-2.5 px-4 md:px-8 flex items-center justify-between sticky top-0 z-50">
      ...
      <div className="hidden md:flex items-center gap-0.5">
        {(isClient || isAgent) && (
          <button
            onClick={() =>
              switchView(isAgent ? 'agentDashboardView' : 'clientDashboardView')
            }
            className={`nav-link-white ${
              isActive(['clientDashboardView', 'agentDashboardView']) ? 'active' : ''
            }`}
          >
            Dashboard
          </button>
        ...
        {(isClient || isAgent) && (
          <button
            ...
            className={`nav-link-white ${
              isActive(['schemesView', 'categoryListView', 'formView', 'successView'])
                ? 'active'
                : ''
            }`}
          >
        ...
        {isAdmin && (
          <button
            ...
            className={`nav-link-white ${
              isActive(['adminDashboardView']) ? 'active' : ''
            }`}
          >
            Dashboard
          </button>
        ...
        <button
          ...
          className={`nav-link-white ${
            isActive(['submissionsView']) ? 'active' : ''
          }`}
        >
        ...
        {unreadNotifs > 0 && (
          <button
            ...
            className="relative p-2 text-[#64748B] hover:text-[#2563EB] rounded-lg transition-all duration-200 hover:bg-[#F0F5FF]"
          >
            <svg
              className="w-5 h-5"
              fill="none"
            ...
            <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-md">
              {unreadNotifs}
            </span>
          </button>
        )}
        ...
        <div className="flex items-center gap-2 pl-2 ml-1 border-l border-[#E2E8F0]">
          ...
          <button
            ...
            className="p-1.5 text-[#64748B] hover:text-[#2563EB] rounded-lg transition-all duration-200 hover:bg-[#F0F5FF]"
          >
            <svg
              className="w-4 h-4"
              fill="none"
          ...
          <button
            ...
            className="p-1.5 text-[#94A3B8] hover:text-[#EF4444] rounded-lg transition-all duration-200 hover:bg-[#FEF2F2]"
          >
            <svg
              className="w-4 h-4"
              fill="none"
